def tots_avgs_t(data):
    import datetime
    from max_temp_month import month_max
    ...
    date_now= datetime.date.today()
    #mx_t = max_t(data)
    #print("maximum temp", mx_t)
    
    #temperature totals averages       
    result_t=[]
    for each in all_months:
        ...
        if each_num <= this_month:
            result_t.append(new_row)
            
    max_temp_data = month_max(data)
    
            
    return  result_t, year_t, max_temp_data
